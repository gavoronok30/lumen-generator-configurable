...
/**
 * Class Update{{ $entityName }}Handler
 * {{ '@' }}package App\Application\{{ $entityName }}
 */
class Update{{ $entityName }}Handler implements Handler
{
    /**
    ...
    /**
     * Update{{ $entityName }}Handler constructor.
    ...
    /**
     * {{ '@' }}param Update{{ $entityName }}|Command $command
     * {{ '@' }}return {{ $entityName }}
     */
...
@if($field->isMultiLanguage())
        ${{ \Illuminate\Support\Str::camel($entityName) }}->replaceTranslations('{{ $field->getField() }}', $command->get{{ \Illuminate\Support\Str::ucfirst($field->getVariable()) }}());
@else
